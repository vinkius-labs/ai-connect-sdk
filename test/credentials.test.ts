...
describe('CredentialsHandle', () => {
  const catalogRoute: Route = {
    ...
    respond: () => ({
      body: { data: { id: 'github', slug: 'github', credential_schema: { GITHUB_TOKEN: { type: 'api_key', required: true } } } },
    }),
  };
  ...
  it('wraps set(values) into the { credentials } envelope and addresses by external_id', async () => {
    const { vinkius, calls } = makeVinkius([
      listRoute,
      {
        ...
        respond: () => ({ body: { schema: { GITHUB_TOKEN: { type: 'api_key' } }, configured: { GITHUB_TOKEN: true } } }),
      },
    ]);
    const status = await vinkius.user('customer-123').connector('github').credentials.set({ GITHUB_TOKEN: 'ghp_x' });
